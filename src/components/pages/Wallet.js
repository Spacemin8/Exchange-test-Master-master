...
import BNBLogo from "../../assets/image/bsc.png"
...
function Wallet() {
  ...
  const fetchData = async () => {
    try {
        setAuthToken(localStorage.jwtToken)
        const ethResponse = await axios.post(serverUrl +'wallets/getCurrentPrice', { sym: 'ETH' });
        setAuthToken(localStorage.jwtToken)
        const bnbResponse = await axios.post(serverUrl +'wallets/getCurrentPrice', { sym: 'BNB' });

        if (ethResponse) {
          console.log(ethResponse.data.data);
            setEthPrice(ethResponse.data.data);
        }

        if (bnbResponse) {
             setBNBPrice(bnbResponse.data.data);
        }
                    ...
                    <div> 
                      <div className="text-xl myColor1 p-0">
                        ...
                        <div className="flex items-center mt-2">
                          <img src={ETHLogo} alt="ETH" className="inline-block mr-2" width={20} height={20} />
                          <span className="text-sm font-bold myColor1">ETH: ${ethPrice} USD</span>
                        </div>
                        <div className="flex items-center mt-2">
                          <img src={BNBLogo} alt="BNB" className="inline-block mr-2" width={20} height={20} />
                          <span className="text-sm font-bold myColor1">BNB: ${bnbPrice} USD</span>
                        </div>
                    </div>
                  </div>
